#!/bin/bash -l
# ============================================================================
# STAGE 2b — same as stage 2, but the confidence loss is PEPTIDE-DOMINATED.
#
# --plddt-peptide-ratio 50 : peptide:MHC weight INSIDE the pLDDT cross-entropy only.
#   At the previous peptide_weight=5, a 9-mer against 181 MHC residues received just
#   5*9/(5*9+181) = 19.9% of the confidence loss — so 80% of the pLDDT gradient was spent
#   on MHC confidence, which is easy, nearly constant, and not the signal a user needs.
#   At 50:1 the peptide gets 71.3%. STRUCTURAL losses are untouched (that is --peptide-
#   weight, a separate knob), so this changes only where the confidence signal is spent.
#
#   Verified on a constructed batch (model confidently RIGHT on MHC, WRONG on peptide):
#     ratio None/5:1 -> plddt_ce 1.6092   (peptide error diluted)
#     ratio 50:1     -> plddt_ce 5.7215   (peptide error dominates)
#
#   NOTE the masked mean is normalised by the weight sum, so changing the ratio changes
#   the CE MAGNITUDE and therefore the lambda balance. Re-measured rather than assumed —
#   see measure_plddt_balance.py --plddt-peptide-ratio 50.
#
# Starting point: stage 2 reached peptide pLDDT correlation +0.774 (from +0.272),
# error-discrimination -0.61, MAE 15.07 -> 5.76, with val-matched pep-RMSD holding at
# 1.32 A. This run pushes the confidence signal onto the peptide specifically.
#
...
EPOCHS=${EPOCHS:-5}
LR=${LR:-2e-4}
SEED_CKPT=${RESUME:-checkpoints_mm1/ARCHIVE/stage2_best.pt}
...
PYCHK
    ...
    $PY -m torch.distributed.run --nnodes=1 --nproc_per_node=${NGPU} \
        ...
        --plddt-w ${PLDDT_W:-0.01} --hasmig-weight 1.0 --struct-quality-weight \
        --plddt-peptide-ratio ${PEP_RATIO:-50} \
